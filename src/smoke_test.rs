...
fn handle_con_srv(con: tokio::net::TcpStream) {
    tokio::task::spawn(async move {
        ...
        let con = config.accept(con).await.unwrap().into();
...
fn handle_con_cli(con: tokio::net::TcpStream) {
    tokio::task::spawn(async move {
        ...
        let con = config.connect(name, con).await.unwrap().into();
        let (con, _) = tokio_tungstenite::client_async("wss://localhost", con).await.unwrap();
        let con = WsFramed::new(con);
        handle_tls_con(con);
    });
}
